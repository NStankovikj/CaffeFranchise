...
<!--
		...
		Amendment:	Addition of conversions for EU specific data fields, fixes for Drug not administered
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:hl7="urn:hl7-org:v3" xmlns:mif="urn:hl7-org:v3/mif" exclude-result-prefixes="hl7 xsi xsl fo mif">
	...
	<xsl:template name="MedicinalProduct">
		...
		<xsl:if test="string-length(hl7:kindOfProduct/hl7:code/@code) > 0">
	...
	<xsl:template match="hl7:substanceAdministration" mode="drug-dosage-information1">
		...
		<xsl:choose>
			<xsl:when test="hl7:effectiveTime//hl7:period/@unit = '{cyclical}' or hl7:effectiveTime//hl7:period/@unit = '{as necessary}' or hl7:effectiveTime//hl7:period/@unit = '{total}'">
	...
	<xsl:template match="hl7:ingredient" mode="drug-additional1">
		<xsl:choose>
			...
			<xsl:when test="@classCode='MECH'">
				<xsl:text>Device (</xsl:text>
				<xsl:value-of select="hl7:ingredientSubstance/hl7:name"/>
				<xsl:text>): </xsl:text>
				<xsl:if test="string-length(hl7:ingredientSubstance/hl7:code/@code) > 0">
					<xsl:text>DeviceID: </xsl:text>
					<xsl:value-of select="hl7:ingredientSubstance/hl7:code/@code"/>
					<xsl:text> (</xsl:text>
					<xsl:value-of select="hl7:ingredientSubstance/hl7:code/@codeSystem"/>
					<xsl:text>-</xsl:text>
					<xsl:value-of select="hl7:ingredientSubstance/hl7:code/@codeSystemVersion"/>
					<xsl:text>)</xsl:text>
				</xsl:if>
				<xsl:text>; </xsl:text>
				<xsl:for-each select="../../../../hl7:outboundRelationship2/hl7:substanceAdministration/hl7:consumable/hl7:instanceOfKind/hl7:productInstanceInstance/hl7:part">
					<xsl:if test="string-length(hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:name)"> 0">
							<xsl:text>Device: </xsl:text>
						<xsl:value-of select="hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:name"/>
					</xsl:if>
					<xsl:if test="string-length(hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:code/@code)"> 0">
							<xsl:text> (DeviceID: </xsl:text>
						<xsl:value-of select="hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:code/@code"/>
						<xsl:text>;</xsl:text>
						<xsl:value-of select="hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:code/@codeSystem"/>
						<xsl:text>-</xsl:text>
						<xsl:value-of select="hl7:partDeviceInstance/hl7:asInstanceOfKind/hl7:kindOfMaterialKind/hl7:code/@codeSystemVersion"/>
						<xsl:text>)</xsl:text>
					</xsl:if>
					<xsl:if test="string-length(hl7:partDeviceInstance/hl7:lotNumberText) > 0">
						<xsl:text>Device lot: </xsl:text>
						<xsl:value-of select="hl7:partDeviceInstance/hl7:lotNumberText"/>
						<xsl:text>; </xsl:text>
					</xsl:if>
				</xsl:for-each>
			</xsl:when>
